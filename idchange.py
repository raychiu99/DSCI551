...
def handle_request(collection_name,key):
	...
	if request.method == 'PUT':
		...
		response_data = data
		response_data['_id'] = str(result.upserted_id) if result.upserted_id else oid
...
#		name = data["name"]
		...
		socketio.emit('data_deleted', {'event': 'deleted', 'user_id': oid})
...
#	collection_name = re.split('[./]', path)[0]
	...
	if request.method == 'GET':
		if "orderBy" in query_params:
			...
			response_data = json.loads(json_util.dumps(result))
